...
import com.bitwaffle.guts.graphics.font.GdxFont.GdxFontData;
...
public class FontCache {
	...
	public FontCache (GdxFont font, boolean integer) {
		this.font = font;
	...
	private float addToCache (CharSequence str, float x, float y, int start, int end) {
		float startX = x;
		GdxFont font = this.font;
		com.bitwaffle.guts.graphics.font.GdxFont.Glyph lastGlyph = null;
		GdxFontData data = font.data;
		if (data.scaleX == 1 && data.scaleY == 1) {
			...
			while (start < end) {
				...
				com.bitwaffle.guts.graphics.font.GdxFont.Glyph g = data.getGlyph(ch);
				if (g != null) {
	...
	private void addGlyph (com.bitwaffle.guts.graphics.font.GdxFont.Glyph lastGlyph, float x, float y, float width, float height) {
		float x2 = x + width;
		float y2 = y + height;
		final float u = lastGlyph.u;
		final float u2 = lastGlyph.u2;
		final float v = lastGlyph.v;
		final float v2 = lastGlyph.v2;
	...
	public TextBounds addMultiLineText (CharSequence str, float x, float y, float alignmentWidth, HAlignment alignment) {
		...
		int numLines = 0;
		while (start < length) {
			int lineEnd = GdxFont.indexOf(str, '\n', start);
			float xOffset = 0;
